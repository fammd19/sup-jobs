...
class Candidate (db.Model, SerializerMixin):
    ...
    def validate_email(self, key, email):
        ...
        if Candidate.query.filter(Candidate.email == email).first():
            raise ValueError("This email is already connect to another candidate account")
...
class Company (db.Model, SerializerMixin):
    ...
    def validate_email(self, key, admin_email):
        ...
        if Company.query.filter(Company.admin_email == admin_email).first():
            raise ValueError("This email is already connect to another company account")
...
class Job (db.Model, SerializerMixin):
    ...
    archived_job = db.Column(db.Boolean, default=False)
