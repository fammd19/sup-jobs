...
export default function CoJobs({ candidate, company }) {
    ...
    useEffect(() => {
        if (id) {
            // If the logged-in company matches the company ID in the URL, fetch all jobs
            const endpoint = company && company.id === parseInt(id, 10)
                ? `/api/jobs/company/${id}`  // Full list of jobs including archived
                : `/api/jobs/company/${id}/live`;  // Only live jobs
